...
import org.fh.service.tools.CodeEditorService;
...
public class CodeEditorController extends BaseController {
	...
	public Object getCode() throws Exception{
		...
		String code = FileUtil.readFileAllContent("/admin/template/ftl/"+type+"/"+ftlNmame+".ftl");	//从原始模版获取
		map.put("code", code);
	...
	public Object save() throws Exception{
		...
		String type = pd.getString("type");
		String ftlNmame = pd.getString("ftlNmame");
		FileUtil.writeFileCR("/admin/template/ftl/"+type+"/"+ftlNmame+".ftl",codeTxt);	//写入到文件
		pd.put("TYPE", type);
	...
	public Object reduction() throws Exception{
		...
		if("fromHistory".equals(msg)){
			...
			code = FileUtil.readFileAllContent("/admin/template/ftl_backups/"+type+"/"+ftlNmame+".ftl");//从原始模版获取
		}
		FileUtil.writeFileCR("/admin/template/ftl/"+type+"/"+ftlNmame+".ftl",code);						//写入到现在模版
		map.put("code", code);
		map.put("result", errInfo);
		return map;
	}
